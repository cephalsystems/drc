...
class TeleopLimb:
    ...
    def solve(self, tf_target):
        with self._lock:
            ...
            success = self._ikv.CartToJnt(self._q_current, x_vel, q_desired)
            if (success >= 0):
                self._x_desired = x_desired
                self._q_desired = q_desired

                q_max = max(abs(q_desired[i]) for i in xrange(0, self._num_joints))
                for i in xrange(0, self._num_joints):
                    self._q_desired[i] /= q_max
    ...
    def populate(self, atlas_command):
        with self._lock:
            ...
            for idx in xrange(0, self._num_joints):
                atlas_command.velocity[self._idx_chain[idx]] = 6.0 * self._q_desired[idx]
    ...
    def clear(self, atlas_command):
        with self._lock:
            # Fill in relevant components of command to atlas
            for idx in xrange(0, self._num_joints):
                atlas_command.velocity[self._idx_chain[idx]] = 0.0
