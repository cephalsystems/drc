...
#ifndef KDLCHAINIKSOLVERPOS_NR_JL_WE_HPP
#define KDLCHAINIKSOLVERPOS_NR_JL_WE_HPP
...
namespace KDL
{
	...
	*/
	class ChainIkSolverPos_NR_JL_WE : public ChainIkSolverPos
	{
		public:
			/**
			...
			* @return
			*/
			ChainIkSolverPos_NR_JL_WE(const Chain& chain, const JntArray& q_min, const JntArray& q_max, 
				ChainFkSolverPos& fksolver, ChainIkSolverVel& iksolver,
				unsigned int maxiter = 100, double eps = 1e-6);
			~ChainIkSolverPos_NR_JL_WE();
			...
			*
			* For instance, if you need to reach some frame but you are not interested in some 
